...
namespace project.BLL.Concrete
{
    ...
    public class PostService : IPostService
    {
        ...
        /// <param name="currentUserId">The ID of the user whose news feed posts will be retrieved.</param>
        /// <returns>A collection of Post objects representing the posts for the news feed.</returns>
        public async Task<IEnumerable<Post>> GetRandomPostsForNewsFeedAsync(string currentUserId)
        {
            var posts = (await GetAllPostsAsync()).Where(p => p.UserId != currentUserId).ToList();
            posts.Shuffle();
        ...
        /// <summary>
        /// Delete a posts of a user asynchronously.
        /// </summary>
        /// <param name="postId">The ID of the post will be deleted.</param>
        /// <returns>Task representing the asynchronous operation.</returns>
        public async Task DeleteUserPostAsync(string postId)
        {
            var post = await _postDal.GetAsync(p => p.Id == postId);
            if(post != null)
                await _postDal.DeleteAsync(post);
        ...
        /// <param name="userId">The ID of the user whose post likes will be deleted.</param>
        /// <returns>Task representing the asynchronous operation.</returns>
        public async Task EditPostAsync(string postId, string description)
        {
            var post = await _postDal.GetAsync(p => p.Id == postId);
            if (post != null)
            {
